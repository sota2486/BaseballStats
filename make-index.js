...
async function bootFromData(){
  ...
  for(const name of data){
    try{
      ...
      if(!res.ok){ missing.push(url(name)+" -> HTTP "+res.status); continue; }
      if(ingest(await res.text(), name)) DATA.files.push(name);
      else missing.push(url(name)+" -> 中身が result / record / play のどれでもない");
    }catch(e){
      missing.push(url(name)+" -> "+(e && e.message ? e.message : e));
    }
  }
